...
loss_df <- data.frame(epoch = integer(), loss = numeric())
...
for (epoch in 1:epochs) {
  ...
  loss <- mean((y_train - output_layer_output)^2)
  print(paste("Epoch:", epoch, "Loss:", loss))
  loss_df <- rbind(loss_df, data.frame(epoch = epoch, loss = loss))
  
  # Backpropagation
  output_error <- output_layer_output - y_train
  output_delta <- output_error * sigmoid_derivative(output_layer_output)
  hidden2_error <- output_delta %*% t(weights_hidden2_output)
  hidden2_delta <- hidden2_error * sigmoid_derivative(hidden_layer_output2)
  hidden1_error <- hidden2_delta %*% t(weights_hidden1_hidden2)
  hidden1_delta <- hidden1_error * sigmoid_derivative(hidden_layer_output1)
...
loss_df%>%ggplot(aes(epoch, loss)) + geom_line() + geom_point()
